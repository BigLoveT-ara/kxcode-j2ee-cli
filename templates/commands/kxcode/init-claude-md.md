...
1. **判断能否执行**
   ...
   如果项目中`.kxcode`不存在，告知项目未使用`kxcode-j2ee-cli`进行初始化，立即退出流程。
...
5. **扫描工具类并输出**
   ...
   生成Api使用的速查表输出到`docs/framework/project-utils-mehtod.md`文件中
...
7. **定时任务分析**
   ...
   如果项目中`@Schema`等定时任务的方法，请分析并输出，输入格式如下：
   ```
   |定时任务|触发频率|完整方法名|
   |---|---|---|
...
8. **CLAUDE文件输出**
   ...
   读取`.kxcode/CLAUDE_TEMPLATE.md`文件已文件内容作为模板，结合上述步骤中的输出部分替换模板中的描述。
   已经有内容的模块，请不要进行任何改动,未标明需要填充的模块请不要私自添加。
   如果没有需要填充的内容可以进行模块的删除。
