---
name: kxcode-create-business-api
...
- 如果有返回值 → 确认用户希望返回的变量的类型和Key，并在最终返回时已`ServiceResponse<BrainResultDTO[]>`进行构建
...
4. **数据库设计确认后如果出现了需求变更，需要同步更新设计三件套（字段文档、SQL、Entity）**s
5. **接口签名必须严格使用 `BrainSessionDTO` 和 `ServiceResponse<BrainResultDTO[]>`，不得自行替换**
6. 对于`BrainSessionDTO`的字段和内容不理解时可以参考`docs/framework/brain-session.md`文档进行理解
7. 需求模糊时，主动提问，不得假设需求后直接编码